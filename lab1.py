        ...
        for n in neighbors:
            ...
            if n not in path:
                ...
                for i in range(1, len(path_copy) - 1):
                    g += graph.get_edge(path_copy[i], path_copy[i + 1]).length
                h = graph.get_heuristic(n, goal)
    ...
    for i in range(len(agenda) - 1):
        for j in range(0, len(agenda) - i - 1):
            ...
            if agenda[j][0] == agenda[j + 1][0]:
                ...
                # number of visited nodes comparison
                if len(agenda[j]) > len(agenda[j + 1]):
                    agenda[j], agenda[j + 1] = agenda[j + 1], agenda[j]

                elif len(agenda[j]) == len(agenda[j + 1]):

                    # node priority comparsion
                    if agenda[j] > agenda[j + 1]:
                        agenda[j], agenda[j + 1] = agenda[j + 1], agenda[j]
